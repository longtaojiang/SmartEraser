...
You can evaluate the SmartEraser model by running the following command:
```
cd Model_framework
bash inference_dis.sh
```
## Local Demo
We also provide a inference demo `app_remove.sh`. You can run the following command to inference a single image with a hand-drawn mask:
```
cd Model_framework
bash app_remove.sh
```
